    ...
    return new Promise((resolve, reject) => {
        ...
        (async () => {
            try {
                ...
                // Парсим объекты - ИСПРАВЛЕНИЕ: добавляем отсутствующий код
                ...
                // Проверяем, что arScene доступен
                if (!arScene) {
                    throw new Error('A-Frame <a-scene> элемент не найден. Невозможно инициализировать MindAR.');
                }
                
                // Добавляем слушатель успешной инициализации
                arScene.addEventListener('mindar-image-initialized', () => {
                    console.log('✅ MindAR: Цели загружены. AR-сцена готова к отслеживанию.');
                    notifyStateChanged('ready');
                    resolve(); // Разрешаем Promise
                }, { once: true });

                // Добавляем слушатель ошибок
                arScene.addEventListener('mindar-image-error', (evt) => {
                    const errorMessage = evt.detail?.error || 'Ошибка инициализации MindAR. Проверьте путь к файлу targets.mind.';
                    console.error('❌ Ошибка MindAR:', errorMessage);
                    notifyError(errorMessage);
                    reject(new Error(errorMessage));
                }, { once: true });

                // Если событие уже произошло, разрешаем Promise сразу
                // (для случаев, когда инициализация уже завершена)
                setTimeout(() => {
                    if (mindarSystem && mindarSystem.el && mindarSystem.el.hasLoaded) {
                        console.log('✅ MindAR уже инициализирован');
                        notifyStateChanged('ready');
                        resolve();
                    }
                }, 1000);
